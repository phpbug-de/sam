...
if (!$box_smarty->is_cached(CURRENT_TEMPLATE.'/boxes/box_menu.html', $cache_id) || !$cache) {
	...
	$content_string='<ul class="nav nav-tabs nav-stacked">';
	while ($content_data = xtc_db_fetch_array($content_query, true)) {
		$SEF_parameter = '';
		if (SEARCH_ENGINE_FRIENDLY_URLS == 'true')
			$SEF_parameter = '&product='.xtc_cleanName($content_data['content_title']);

		$content_string .= '<li><a href="'.xtc_href_link(FILENAME_CONTENT, 'coID='.$content_data['content_group'].$SEF_parameter).'"><i class="icon-chevron-right"> </i> <strong>'.$content_data['content_title'].'</strong></a></li>';
	}
...
if (!$cache) {
    $box_information = $box_smarty->fetch(CURRENT_TEMPLATE.'/boxes/box_menu.html');
} else {
    $box_information = $box_smarty->fetch(CURRENT_TEMPLATE.'/boxes/box_menu.html', $cache_id);
}
//EOF - DokuMan - 2010-02-28 - fix Smarty cache error on unlink

$smarty->assign('box_MENU', $box_information);
?>